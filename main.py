...
def load_truck(truck, selected_package=None):
    if len(truck.packages_on_board) == truck.max_packages:
        return print("Truck is too full to load any more packages")
    if selected_package is None:
        return print("No package selected to load")
    if selected_package:
        package_index = total_available_package_list.index(selected_package)  # gets index of package in the list
        package_to_load = total_available_package_list.pop(package_index)  # removes package form list to load it
...
def auto_load_truck(truck):
    ...
    if truck.clock < datetime.datetime(1990, 1, 1, 9, 5, 0, 0):      # if time before 9:05 AM can't 6, (9?), 25, 28, 32
        effectively_available_package_list = [package for package in total_available_package_list if
                                             package.id not in package_ids_not_before_9]
    ...
    if truck.name == 'truck1':
        packages_for_only_truck1 = [package for package in effectively_available_package_list if
                                    package.id not in package_ids_only_truck2]

        while packages_for_only_truck1:
            next_package_to_load = find_next_package(packages_for_only_truck1, truck.current_last_package())
            package_to_load_index = packages_for_only_truck1.index(next_package_to_load)
            packages_for_only_truck1.pop(package_to_load_index)
    ...
    if truck.name == 'truck2':
        packages_for_only_truck2 = [package for package in effectively_available_package_list if
                                    package.id not in package_ids_only_truck1]

        effectively_available_package_list.append(packages_for_only_truck2)
    ...
    while len(truck.packages_on_board) < truck.max_packages and effectively_available_package_list:
        ...
        total_available_package_list.pop(package_to_load_index)           # remove from global total available packages
        load_truck(truck, next_package_to_load)
...
def reorder_packages(truck):
    temp_list = truck.packages_on_board
    truck.packages_on_board.clear()
    auto_load_truck(truck, temp_list)
...
# create the 2 trucks (effectively since only 2 drivers)
truck1 = Truck('truck1')
truck2 = Truck('truck2')
